...
/**
 * 简化版系统主题的暗黑模式背景/文字变量
 * Wot UI 2.x 的 ConfigProviderThemeVars 未声明这些字段，由项目自行扩展
 */
export interface SystemDarkThemeVars {
...
/**
 * 注入 wd-config-provider 的项目主题变量
 * = Wot UI 官方变量 + 主色阶 + 暗色背景/文字 + colorTheme
 */
export type AppThemeVars = ConfigProviderThemeVars &
  Partial<ThemePrimaryShades> &
  Partial<SystemDarkThemeVars> & {
    colorTheme?: string;
  };
...
/** 简化版系统主题：暗黑模式背景/文字色阶 */
export const systemDarkThemeVars: SystemDarkThemeVars = {
...
/** 构建主题色阶变量（供 ConfigProvider 注入） */
export function buildThemeVars(color: ThemeColorOption): ThemePrimaryShades {
  return {
    ...color.primaryShades,
  };
}

/** 构建简化版系统主题变量：暗黑色阶 + 默认主题色阶 */
export function buildSystemThemeVars(color: ThemeColorOption = themeColorOptions[0]): AppThemeVars {
  return {
    ...systemDarkThemeVars,
