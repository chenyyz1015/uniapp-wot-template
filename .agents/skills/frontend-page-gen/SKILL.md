...
description: 全流程前端页面生成器 — 当用户需要创建、生成、新增、搭建页面时触发。覆盖需求分析 → 设计系统检查与建立 → 选择设计工具（Pencil MCP / Figma MCP）→ 设计稿 → 审核确认 → 完整代码实现（页面 + API + Store + 路由 + i18n）→ 验证 → impeccable 细节审核的全链路。适用于 UniApp 移动端页面（列表、表单、详情）和 H5 落地页。即使用户只说"我要做一个XX页面"也使用此技能。
---
...
1. 什么类型的页面？（移动端列表 / 表单 / 详情 / H5 落地页 / 其他）
2. 这个页面解决什么问题？有哪些核心操作？
```

**移动端管理类补充**（按需选取）：
- 页面包含哪些区域？（搜索栏、列表/卡片、分页、弹窗表单、详情页等）
- 是否有 CRUD 操作？后端 API 是否已就绪？
- 列表字段有哪些？哪些可搜索？表单字段、必填项？
- 是否需要权限控制？目标平台（H5 / 微信小程序 / App）？
...
- **主要区域**：搜索栏 + 列表 + 分页 + 新增/编辑弹窗
- **页面路径**：/pages/system/user/index
- **布局**：default
- **API 状态**：已就绪 / mock
- **权限**：需要登录
...
2. **询问用户风格偏好**（常规业务页可跳过，直接走 Wot UI 默认主题）：
   ```
   项目还没有定义设计系统。先确认页面风格：
...
- **进入开发阶段**：H5 落地页 token 写入页面级 SCSS；业务页使用 Wot UI 主题变量（`wd-config-provider` / `theme-vars`），UX 规则作为补充
...
- **移动端业务页**：遵循 Wot UI 规范，使用 `wd-input`、`wd-button`、`wd-cell`、`wd-popup` 等
- **H5 落地页**：颜色/字体/间距必须来自 `MASTER.md` token，禁止硬编码
- **状态覆盖**：空状态、加载态、错误态
- **响应式**：标注 H5 与小程序至少 2 个断点（如 375 / 768）
...
**技术规范（对照 `.claude/rules/`）**：Wot UI 组件使用、UX token 引用、BEM 类名、交互状态完整性
...
> **先读规范再写代码**。生成顺序：类型定义 → API 模块 → i18n → 页面组件 → Store → 页面路由声明。每完成一组告知进度。

### 开发前必读

读取以下文件确认当前项目状态：
- `.claude/rules/code-style.md` `.claude/rules/api-conventions.md` `.claude/rules/css-naming.md` `.claude/rules/permissions.md` `.claude/rules/router.md`
- `src/pages/` 现有页面结构、`src/i18n/locales/zh-CN.json`
- `design-system/MASTER.md`（如果存在，H5 落地页必须读取以提取 token）

### 规范与模板

详细规范速查和审查清单见 **[references/code-conventions.md](references/code-conventions.md)**。
代码模板（CRUD 列表页、表单弹窗、搜索栏、落地页）见 **[references/code-templates.md](references/code-templates.md)**。

Page type 组件/状态参考见 **[references/page-types.md](references/page-types.md)**。

### 核心禁止事项

- 手动 import auto-import 的内容（`vue` API、`vue-i18n`、`useToast`、`useDialog`、`wd-*` 组件、公共组件、`usePermission` 等）
- 模板堆叠 ≥ 3 个非 BEM / 非 shortcuts 的 utility class
- 组件内用 `function` 声明方法（页面内简单 handler 可例外，与现有页面保持一致）
- 使用 `.then()` / `.catch()` 链式调用
- 穿透子组件 `$refs` 或 `$el`
- H5 落地页硬编码颜色/字体（必须来自 MASTER.md token）
- 使用 `v-permission` / `v-role` 自定义指令（统一 `usePermission()` + `v-if`）
...
1. `npm run dev` 启动开发服务器（H5 或目标平台）
2. 访问生成页面，截图
...
3. **判断 register**：业务页 → `product`，H5 落地页 → `brand`，加载对应 reference
4. **执行审核**：
