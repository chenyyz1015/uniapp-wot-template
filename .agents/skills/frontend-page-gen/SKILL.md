...
- **进入开发阶段**：H5 落地页 token 写入页面级 SCSS；业务页使用项目 UI 库的主题变量 / CSS 变量（见 `.claude/rules/code-style.md`），UX 规则作为补充
...
3. **调用 MCP**：`CallMcpTool` → server `user-openai-image` → `gpt_image_generate`
   - 移动端：`size: "1024x1536"`，完整单屏 UI 概念图
   - H5 落地页：`size: "1536x1024"` 或 `"1024x1536"`（长页可生成 2-3 张分段图）
...
- **移动端业务页**：遵循项目 UI 组件库规范（见 `.claude/rules/code-style.md`），选用项目已有的输入框、按钮、列表、弹窗等组件
...
- 页面 `<image>` 引用路径与 `src/static/` 下 openai-image MCP 生成文件一致（如 `/static/illustrations/xxx.png`）
...
- 使用项目禁止的权限指令或模式（见 `.claude/rules/permissions.md`）
