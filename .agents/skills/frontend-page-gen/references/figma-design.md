...
1. **生成布局框架**：用 `use_figma` 创建页面级线框图，只关注整体区块划分和内容层级，暂不设置颜色和字体细节。
   ...
   **移动端业务页**：导航标题 → 搜索栏 → 操作按钮 → 列表/卡片 → 加载更多
   **H5 落地页**：导航栏 → Hero → Features → 内容区块 → CTA → Footer
...
1. **细化内容**（用 `use_figma` 逐个区块更新）：
   ...
   - Wot UI 组件：输入框、按钮、单元格、弹窗等，与项目 `wd-config-provider` 主题变量一致
